...
Foreign Key (unitClass_nameId) references Term(term_id)
);
...
Foreign Key (age_nameId) references Term(term_id)
);
...
player_resources varchar(100) DEFAULT '0,0,0,0,0',	/*cantidad total de recursos que tiene el jugador*/
...
Foreign Key (player_age) references Age(age_id)
);
...
unit_classId int,			/* Superclase de la unidad (Infanteria, caballeria, artilleria... etc)*/
...
Foreign Key (unit_nameId) references Term(term_id),
Foreign Key (unit_startAge) references Age(age_id),
Foreign Key (unit_endAge) references Age(age_id),
Foreign Key (unit_classId) references UnitClass(unitClass_id)
);
...
Foreign Key (resource_nameId) references Term(term_id),
Foreign Key (resource_startAge) references Age(age_id),
Foreign Key (resource_startAge) references Age(age_id)
);
...
Foreign Key (sector_occupantId) references Player(player_id),
Foreign Key (sector_ownerId) references Player(player_id)
);
...
Foreign Key (division_ownerId) references Player(player_id),
Foreign Key (division_sectorId) references Sector(sector_id),
Foreign Key (division_unitId) references Unit(unit_id)
);
...
Foreign Key (buildingClass_nameId) references Term(term_id),
Foreign Key (buildingClass_startAge) references Age(age_id),
Foreign Key (buildingClass_endAge) references Age(age_id)
);
...
Foreign Key (productionMod_resourceId) references Resource(resource_id)
);
...
Foreign Key (battleMod_nameId) references Term(Term_id),
Foreign Key (battleMod_targetClassId) references UnitClass(UnitClass_id)
);
...
Foreign Key (battleModLink_targetId) references Unit(unit_id),
Foreign Key (battleModLink_battleModId) references BattleMod(battleMod_id)
);
...
Foreign Key (building_BuildingClassId) references BuildingClass(BuildingClass_id),
Foreign Key (building_sectorId) references Sector(sector_id)
);
...
Foreign Key (technology_nameId) references Term(term_id),
Foreign Key (technology_startAge) references Age(age_id),
Foreign Key (technology_endAge) references Age(age_id),
Foreign Key (technology_visibleAge) references Age(age_id)
);
...
Foreign Key (technologyLink_technologyId) references Technology(technology_id),
Foreign Key (technologyLink_playerId) references Player(player_id)
);
...
Foreign Key (battle_sectorId) references Sector(sector_id)
);
...
create table Message (
	...
	Foreign Key (message_from) references Player(player_id),
	Foreign Key (message_to) references Player(player_id)
);
/*
...
insert into Age values (1,22,'LinkIMG','0,100,0,0,0'), (2,23,'LinkIMG',NULL), (3,46,'LinkIMG',NULL);
...
insert into BattleModLink values (0, 1, 4);
...
/*INSERT INTO `Player` (`player_id`, `player_nick`, `player_password`, `player_email`, `player_age`, `player_flag`, `player_avatar`, `player_civName`, `player_resources`, `player_lastUpdate`, `player_isLogged`, `player_lastMapOrigin`, `player_lastMapHeight`) VALUES
(1, 'ejp', '6a723ead6ae3c6ebd420446a2e13d340 ', 'bnm', 1, 'ejp_flag.png', 'ejp_avatar.png', 'nmjk,', '100,1000,1000,1000,0', '1327689575', 0, '0,0', 5);*/
